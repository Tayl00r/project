 ...
 *
 */
public class DataPurgeETL_bak20160509 {
	...
	// 数据批量动态插入
	public boolean insertDataBySelectRsBatch(String insertIntoTabColumn,
			                                 String selectRsSql,
		...
		try {
			// execute query
			dcETLHelper.executeQuery(selectRsSql, selectArgs);
			...
			String insertIntoFixedSql = insertIntoTabColumn + " values(" + question + ")";
			...
			while (dcETLHelper.resultSet.next()) {
				...
				if (dcETLHelper.getCurrentSize() == GlobalInfo.MAX_BATCH_SIZE) {
					dcETLHelper.exeBatch(0, false);  // 暂不提交
				}
			} // while
			
			if (dcETLHelper.getCurrentSize() > 0 ){
				dcETLHelper.exeBatch(99, false);  // 暂不提交
			}
			
		} catch (Exception e) {
			returnFlag = false;
			e.printStackTrace();
			LogManager.appendToLog("insertDataBySelectRsBatch() => Exception: " + e.toString(), 
					GlobalInfo.EXCEPTION_MODE);
			throw e;
		} finally {
			dcETLHelper.close();
