 ...
 *
 */
public class DataPurgeETL_bak20160808 {
	...
	public int insertDataBySelectRsBatch(String insertIntoTabColumn,
		...
		try {
			...
			String onDuplicateKeySql = "";
			if (destTabUniqueConstraintColList.size() > 0 || inRulesTabColList.size() > 0) {
				onDuplicateKeyFlag = true;
				onDuplicateKeySql = " on duplicate key update ";
				// 此部分代码可以不需要
				for (int i=0; i<destTabUniqueConstraintColList.size(); i++) {
					onDuplicateKeySql = onDuplicateKeySql + destTabUniqueConstraintColList.get(i).toString() + "=?,";
				}
				// 更新维护的映射值
				for (int i=0; i<inRulesTabColList.size(); i++) {
					onDuplicateKeySql = onDuplicateKeySql + inRulesTabColList.get(i).getDest_tab_column_name() + "=?,";
				}
				//取出多余的末尾分号
				onDuplicateKeySql = onDuplicateKeySql.substring(0, onDuplicateKeySql.length()-1);
			}
			
			String insertIntoFixedSql = insertIntoTabColumn + " values(" + question + ")" + onDuplicateKeySql;
			...
			while (dcETLHelper.resultSet.next()) {
				...
				if (onDuplicateKeyFlag) {
					for (int i=0; i<destTabUniqueConstraintColList.size(); i++) {
						args.add(dcETLHelper.resultSet.getObject( destTabUniqueConstraintColList.get(i).toString() ));
					}
					// 更新维护的映射值
					for (int i=0; i<inRulesTabColList.size(); i++) {
			...
			LogManager.appendToLog("insertDataBySelectRsBatch() [" + destTableOwner + "."+ destTableName + 
					"] => Exception: " + e.toString(), GlobalInfo.EXCEPTION_MODE);
			throw e;
		} finally {
			dcETLHelper.close();
		}
	...
	public IfaceReturns startDataPurgeEngine(IfaceParameters ifaceParameter,
		...
		for (QueryDPInstanceConfigsRsEO rs : dpTabList) {
			...
			if (inRulesTabColList.size() == 0 && notInRulesTabColList.size() == 0) {
						...
						if ( isTabColumnExists(/*rs.getDest_dp_table_owner(),
							...
							if (selectFromColumnList != null && !"".equals(selectFromColumnList)) {
								selectFromColumnList = selectFromColumnList + "," + 
										fromConditionSourTabAlias + "." + tableColList.get(i).getColumn_name();
							} else {
								selectFromColumnList = fromConditionSourTabAlias + "." + tableColList.get(i).getColumn_name();
							}
				...
				for (int j=0; j<inRulesTabColList.size(); j++) {
					...
					if ( isTabColumnExists(/*rs.getDest_dp_table_owner(),
						...
						// 为了便于处理，赋予一个别名
						selectFromColumnList = selectFromColumnList + " AS " + inRulesTabColList.get(j).getSource_tab_column_name();
			...
			// 最终的组装
			String fullDataPurgeSql = insertIntoSql + GlobalInfo.LINE_SEPARATOR + selectSql;
			if (actionFlag.equals(GlobalInfo.DCETL_ENGIN_DO_PURGE) && initDataImpFlag) {
				// 期初数据导入则：改变方式
				fullDataPurgeSql = selectSql;
			}
			...
			if (inRulesTabColList.size() > 0 && !initDataImpFlag) {
				if (onDuplicateKeyUptList != null && !"".equals(onDuplicateKeyUptList)) {
					fullDataPurgeSql = fullDataPurgeSql + GlobalInfo.LINE_SEPARATOR + 
			...
			try {
				...
				String fullDataPurgeSqlBak = fullDataPurgeSql;
				if ( !initDataImpFlag ) {
					// 验证保持原来的逻辑
					dcETLHelper.setAndExecuteDML(fullDataPurgeSql, args);
				} else {
					insertCount = insertDataBySelectRsBatch(insertIntoSql, fullDataPurgeSql, args, 
